...
public class E01JunsuType {
	...
	public static void main(String[] args) {
		...
		/*
		- 동일자료형끼리의 연산은 동일자료형이 되는것이 기분 원칙임.
		...
		/*
			자바컴파일러(CPU)는 정수형상수를 기본적으로 int형으로 간주하기 때문에 위 정수를 메모리에 
			올리는순간 에러가 발생한다. 이때는 접미사(L 혹은 l)을 붙여서 int형 자료가 아닌 long형
			자료임을 알려줘야한다. 이와같이 대입을 위해 입력되는 숫자도 자료형을 기반으로 메모리에 저장되고,
			이를 '리터럴'이라고 한다.
		 */
