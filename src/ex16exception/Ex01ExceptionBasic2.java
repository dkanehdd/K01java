...
-main메소드에서 무시하기위해 throws하더라도 오류가 발생되므로
...
public class Ex01ExceptionBasic2 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			intArr[2] = 300;
			/*
			예외가 발생하는 지점으로, 예외객체가 전달되므로 아래문장은 
			실행되지 않음. catch블럭으로 바로 넘어가게됨.
			 */
		...
		//	today 참조변수가 가지고있는 null값을 toString()메소드를통해 
		//	String 형태로 변환되어 출력된다. 예외는 발생하지 않는다.
		try {
			System.out.println(toDay.getTime());/*
					toDay참조변수가 참조할 객체가 없는 상태이므로
					해당메소드를 호출할 수 없어 예외가 발생된다.
			*/
		}
