...
public class Ex06Define1Predicate {
	...
	public static void main(String[] args) {
		...
		/*
		전달된 객체의 성별이 "남" 인지 판단하여 boolean값을 반환하는 람다식정의.
		test()메소드를 오버라이딩하여 정의하였으므로
		아래와같이 호출할 수 있다.
		 */
		Predicate<Human> pre = (Human h) -> {
			return h.getGander().equals("남");
		};
		...
		double maleAvg = avg(a -> a.getGander().equals("남"), list1);
		System.out.println("남자 평균:"+maleAvg);
		
		//매개변수 객체가 여자일때 true를 반환하는 람다식을 전달함.
		double femaleAvg = avg((Human b)->b.getGander().equals("여"),list1);
