...
public class E07RideAndLoad01 {
	...
	public static void main(String[] args) {
		...
		ref2.rideMethod();//정의된(오버라이딩된)
		ref3.rideMethod();//메소드가 호출된다.
		
		System.out.println("오버로딩 처리된 메소드");
		
		ref3.loadMethod();//각각의 객체에
		ref3.loadMethod(1);//정의된 (오버로딩된)
		...
		C refAddr1 = new C();// C타입 c = C객체 -> 가능 
		B refAddr2 = refAddr1;//B타입 b = C객체 -> 가능
		A refAddr3 = refAddr1;//A타입 a = C객체 -> 가능 
		
		A refId1 = new C();// A타입 = C객체 -> 가능
		//B refID2 = refId1;// B타입 = A객체 -> 불가능(에러발생)
		//C refId3 = refId1;// C타입 = A객체 -> 불가능(에러발생)
		
		//위에서 에러가 난 부분은 아래처럼 다운캐스팅(강제형변환)하면 참조 가능하다
		C refId3 = (C)refId1;
